...
module.exports = (req, res) => {
  ...
  nodemailer.createTestAccount((err, account) => {
    ...
    //     if (err) return response(res, 500, false, 'Erreur Signup Utilisateur')
    //     db.collection('society').find({_id: req.body.idSociety}).toArray((err, result) => {
    //       if (err) return response(res, 500, false, 'Erreur Signup Utilisateur')
    //       if (result.length !== 1) return response(res, 404, false, 'société non trouvé')
